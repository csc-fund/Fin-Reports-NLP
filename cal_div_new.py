    ...
    def get_div_by_year(self):
        ...
        for code in tqdm(self.DIV_TABLE['stockcode'].unique()):
            ...
            for lag_t in self.LAG_PERIOD:  # 最近2次年报的股息
                lag = pd.merge(df_date[['report_year', 'dvd_pre_tax_sum', 'dvd_pre_tax_count', 'ann_date_max']],
                               pd.DataFrame(df_date['report_year'] - lag_t),
                               how='right', on=['report_year'], )

                lag.rename(columns={'report_year': 'report_year_l{}'.format(lag_t),
                                    'dvd_pre_tax_sum': 'dvd_pre_tax_sum_l{}'.format(lag_t),
                                    'dvd_pre_tax_count': 'dvd_pre_tax_count_l{}'.format(lag_t),
                                    'ann_date_max': 'ann_date_max_l{}'.format(lag_t)
                                    }, inplace=True)
...
def get_exp_div(self):
    self.MERGE_TABLE = pd.read_csv('merge.csv')
    # 增加静态计算逻辑列
